# AES256 encryption method in a python script to be passed to JavaFx main class
#
# Sources
...
def gen_key():
    # generate the key needed for AES256 encryption and decryption (symmetric key)
    key = get_random_bytes(16)
    with open("aes256Secretkey.key", "wb") as file:
        file.write(key)

def load_key():
    return open("aes256Secretkey.key", "rb").read()

def encrypt_message(message,key):
    # # Steps for encryption:
    #  1. Create Cipher instance with AES256
    ...
    AES256cipher = AES.new(key, AES.MODE_EAX)
    cipherStore = AES256cipher.nonce

    # writing cipherStore/nonce out to a file to be pulled for decryption
    with open("aes256CipherStore_nonce.txt", "wb") as file:
        file.write(cipherStore)
        print("Export of variable cipherStore aka nonce complete. Proceeding with encryption.")

    encrpyted, tag = AES256cipher.encrypt_and_digest(message)

    # writing out created variable tag needed for validity checking while decryption to ensure authenticity of the message
    with open("aes256EncryptedTag.txt", "wb") as file:
        file.write(tag)
        print("Export of variable tag aka AES256 encrypted text identifier for verification complete. Proceeding with writing out Encrypted message. Returning to Menu.")

    # writing encryption out to a file to be stored for decryption
    with open("aes256Encrypted.txt", "wb") as file:
        file.write(encrpyted)
        print("Export of variable encrypted aka AES256 encrypted text complete. Finished with encryption process. Returning to Menu.")
...
def decrypt_message(message,key,cipherStore,tag):
    ...
    try:
        ...
        # writing encryption out to a file to be stored for decryption
        with open("aes256Decrypted.txt", "wb") as file:
            file.write(decrypt)
...
# 
# decrypt_message(message=open("aes256Encrypted.txt", "rb").read(),key=load_key(),cipherStore=open("aes256CipherStore_nonce.txt", "rb").read(), tag=open("aes256EncryptedTag.txt", "rb").read())
